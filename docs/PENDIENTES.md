...
| A3 | **Línea de crédito compartida / OBO billing** (Meta cobra a TuBot, TuBot al cliente) | 🔴 **BLOQUEANTE — fecha límite 2026-09-30** (des-parkeado 2026-08-31). Desde el 1 de octubre de 2026 Meta cobra los mensajes de servicio y **deja de entregarlos a toda WABA sin medio de pago registrado al 30-09**. Todo cliente sin tarjeta propia en su Business Manager (o sin línea de crédito OBO nuestra) **deja de responder**. Decidir el modelo (tarjeta del cliente vs OBO) y verificar el medio de pago de cada WABA antes del 30-09. | Sí — condición de operación desde octubre | L + ⏳ |
...
| B8 | **Flujos: timeout global de ejecución por flujo** | Bloque 5 dejó lo esencial (rama `si falla`, política por paso, reintentos). ✅ **Notificación por fallo YA está** (evento `workflow.failed` en el catálogo → in-app+email a admins/dueño, anti-spam: 1 aviso por flujo por hora, en `finishRun`). Falta solo: **timeout global configurable por flujo** que cancele un run colgado. | Sí, endurecimiento | M |
