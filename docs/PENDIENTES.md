...
| B8 | **Flujos: timeout global de ejecución + notificación por fallos repetidos** | Bloque 5 dejó lo esencial (rama de error `si falla`, política por paso detener/continuar, reintentos inmediatos). Falta: (1) timeout global configurable por flujo que cancele un run colgado; (2) avisar al tenant vía el catálogo de notificaciones cuando un flujo publicado falla repetidamente (hoy solo emite `workflow.failed`). | Sí, endurecimiento | M |
