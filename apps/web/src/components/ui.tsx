...
export function Button({
  ...
  const base =
    "inline-flex items-center justify-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-600";
  const variants = {
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-card border border-dashed border-slate-300 bg-white/60 px-6 py-10 text-center">
  ...
  return (
    <div className="flex gap-1 overflow-x-auto border-b border-slate-200" role="tablist">
      {tabs.map((t) => (
        ...
        >
          {t.label}
          {t.badge && <span className="ml-1.5 rounded-full bg-slate-100 px-1.5 text-[10px] text-slate-500">{t.badge}</span>}
        </button>
      ))}
    </div>
  );
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className={cn("relative max-h-[90vh] w-full overflow-y-auto rounded-2xl bg-white p-6 shadow-pop", wide ? "max-w-3xl" : "max-w-lg")}>
        <div className="mb-4 flex items-center justify-between">
          {title && <h2 className="text-lg font-semibold">{title}</h2>}
          <button onClick={onClose} aria-label="Cerrar" className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
            <X size={18} />
          </button>
        </div>
        {children}
...
export function Drawer({
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-navy-950/50 backdrop-blur-[2px]" onClick={onClose} />
      <div className="absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto bg-white shadow-pop">
...
export function SecretField({ value, label }: { value: string; label?: string }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-xs">
        <span className="flex-1 truncate">{show ? value : "•".repeat(Math.min(value.length, 28))}</span>
        <button onClick={() => setShow(!show)} aria-label={show ? "Ocultar" : "Mostrar"} className="p-1 text-slate-400 hover:text-slate-600">
      ...
      <div className="pointer-events-none fixed bottom-4 right-4 z-[60] flex w-80 flex-col gap-2" aria-live="polite">
        {items.map((t) => (
          <div
            key={t.id}
            className={cn(
              "pointer-events-auto flex items-start gap-2 rounded-xl border bg-white p-3 text-sm shadow-pop",
              t.kind === "ok" && "border-emerald-200",
