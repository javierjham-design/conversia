  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      <ul className="mt-4 space-y-1">
        {fields.length === 0 && <p className="rounded-lg border border-dashed border-slate-200 p-4 text-center text-sm text-slate-400">Aún no hay campos personalizados.</p>}
        {fields.map((f, idx) => (
          <li key={f.id} className="flex items-center gap-2 rounded-lg border border-slate-100 bg-white px-2 py-1.5">
            <div className="flex flex-col">
              ...
              <button onClick={() => idx < fields.length - 1 && void move(idx, 1)} disabled={idx === fields.length - 1} className="text-slate-300 hover:text-slate-600 disabled:opacity-30"><ArrowDown size={11} /></button>
            </div>
            <span className="min-w-0 flex-1">
              <span className="text-sm font-medium">{f.label}</span>
              <span className="ml-2 font-mono text-[10px] text-slate-400">{f.key}</span>
              {f.required && <span className="ml-2 text-[10px] text-amber-600">obligatorio</span>}
            </span>
            <span className="shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">{TYPES.find(([t]) => t === f.type)?.[1] ?? f.type}</span>
            <span className="w-20 shrink-0 text-right text-[10px] text-slate-400">{f.valuesCount} con valor</span>
    ...
    <Modal open onClose={onCancel} title={field ? `Editar «${field.label}»` : "Nuevo campo de contacto"}>
      <div className="space-y-2">
        <label className="block text-sm">
          <span className="text-xs text-slate-500">Nombre del campo</span>
          <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="p. ej. Previsión" className={input} />
        </label>
        <label className="block text-sm">
          <span className="text-xs text-slate-500">Tipo</span>
          <select value={type} onChange={(e) => setType(e.target.value)} className={`${input} bg-white`} disabled={!!field}>
            {TYPES.map(([v, l]) => (<option key={v} value={v}>{l}</option>))}
          </select>
        </label>
        {type === "select" && (
          <label className="block text-sm">
            <span className="text-xs text-slate-500">Opciones (separadas por coma)</span>
