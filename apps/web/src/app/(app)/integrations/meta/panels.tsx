    ...
    <Modal open={open} onClose={onClose} title="Conectar Meta Business Suite" wide>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          {!connected ? (
            <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-500">Primero completa el paso 1 (conectar).</p>
          ) : (
            assetGroups.map(([label, assets]) => (
              <div key={label}>
                <p className="mb-1.5 text-[13px] font-semibold text-slate-500">{label}</p>
                {assets.length === 0 ? (
                  <p className="text-xs text-slate-400">Sin activos detectados{data.connection?.mode === "MANUAL" ? " — conecta números en Canales" : ""}.</p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {assets.map((a) => (
                      <label
                        ...
                        className={cn(
                          ...
                          a.enabled ? "border-brand-300 bg-brand-50" : "border-slate-200",
                        )}
                      >
                        <input
                          type="checkbox"
                        ...
                        />
                        <span>{a.name}</span>
                        <span className="font-mono text-[10px] text-slate-400">{a.externalId}</span>
                      </label>
                    ))}
                  </div>
                )}
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          {[
            ...
            <div key={name as string} className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
              <div>
              ...
              <div className="text-right">
                ...
                <p className="mt-0.5 text-[10px] text-slate-400">{hint as string}</p>
              </div>
            </div>
          ))}
      ...
      {step === 4 && (
        <div className="space-y-4">
          <p className="text-sm text-slate-600">Ejecuta pruebas reales por el pipeline completo:</p>
          <div className="flex flex-wrap gap-2">
            <Button
              variant="secondary"
              disabled={busy}
          ...
          {testLog.length > 0 && (
            <ul className="space-y-1 rounded-xl bg-slate-50 p-3 text-xs">
              {testLog.map((l, i) => (
                <li key={i} className={l.startsWith("✖") ? "text-red-600" : "text-slate-600"}>{l}</li>
              ))}
            </ul>
          )}
          <div className="flex justify-between border-t border-slate-100 pt-3">
            <p className="text-xs text-slate-400">Los resultados quedan en la pestaña Actividad.</p>
    ...
    <div className="space-y-4">
          ...
          {rows.map((row, i) => (
            <div key={i} className="flex items-center gap-2">
              <input
                ...
                className="w-48 rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs"
              ...
              <button onClick={() => setRows((r) => r.filter((_, idx) => idx !== i))} aria-label="Quitar fila" className="p-1 text-slate-300 hover:text-red-500">
                <Trash2 size={15} />
              </button>
            </div>
          ))}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="block text-sm font-medium">
          ...
          <select value={statusCode} onChange={(e) => setStatusCode(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">
            {leadStatuses.map((s) => (
        ...
        <label className="block text-sm font-medium">
          ...
          <input value={tags} onChange={(e) => setTags(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
        </label>
      </div>
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2">
        <label className="block text-sm font-medium">
          ...
          <input value={datasetId} onChange={(e) => setDatasetId(e.target.value)} placeholder="123456789012345" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-sm" />
        </label>
        <label className="block text-sm font-medium">
          Test event code (opcional)
          <input value={testEventCode} onChange={(e) => setTestEventCode(e.target.value)} placeholder="TEST12345" className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-sm" />
        </label>
      </div>
          ...
          {rules.map((rule, i) => (
            <div key={i} className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 p-2">
              <select
                ...
                className="rounded-lg border border-slate-300 bg-white px-2 py-1.5 font-mono text-xs"
              >
              ...
              <input
                ...
                placeholder="CLP"
                className="w-16 rounded-lg border border-slate-300 px-2 py-1.5 text-xs uppercase"
                aria-label="Moneda"
              />
              ...
              <button onClick={() => setRules((r) => r.filter((_, idx) => idx !== i))} aria-label="Quitar regla" className="ml-auto p-1 text-slate-300 hover:text-red-500">
                <Trash2 size={15} />
              </button>
            </div>
          ))}
