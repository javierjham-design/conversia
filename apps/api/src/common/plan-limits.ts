...
/**
 * Uso de mensajes de plantilla facturables del período (informativo). El cobro es
 * PREPAGO por la bolsa (message_wallets); ya no hay excedente post-pago. `included`
 * viene del plan (features.templateMessages). Cuenta usage_events `whatsapp_message`.
 */
export async function getTemplateUsage(tx: TenantTx): Promise<TemplateUsage> {
  ...
  return { used: agg._count._all, included, periodStart: periodStart.toISOString() };
}
