...
export class IntegrationsController {
  ...
  overview() {
    ...
    return this.prisma.withTenant(ctx.organizationId, async (tx) => {
      const [scheduling, webhooks, credentials, meta, channels, events24h, deliveries, lastEvent] = await Promise.all([
      ...
      return {
        ...
        catalog: [
          ...
          { key: "email", name: "Correo electrónico", category: "datos", status: "proximamente", description: "Notificaciones y resúmenes por correo.", capabilities: ["Notificaciones"] },
