    ...
    return this.prisma.withTenant(ctx.organizationId, async (tx) => {
      ...
      return {
        ...
        catalog: [
          ...
          { key: "google_calendar", name: "Google Calendar", category: "agenda", status: platformGoogleReady ? "disponible" : "config_pendiente", description: "Espejo de tus citas de TuBot en el calendario que elijas (OAuth de Google).", capabilities: ["OAuth", "Espejo de citas", "Cancelaciones"] },
          ...
          { key: "webhooks", name: "Webhooks salientes", category: "datos", status: "disponible", description: "Recibe los eventos de TuBot en tus sistemas, firmados HMAC.", capabilities: ["14 eventos", "Reintentos", "Firma HMAC"] },
          ...
          { key: "zapier", name: "Zapier", category: "datos", status: "disponible", description: "Asistente guiado: trigger con nuestros webhooks + acciones con la API de TuBot (sin app nativa).", capabilities: ["Asistente", "Webhook + API key", "Plantillas"] },
          ...
          { key: "hubspot", name: "HubSpot", category: "crm", status: platformHubspotReady ? "disponible" : "config_pendiente", description: "Sincroniza tus contactos de TuBot a HubSpot (unidireccional, sin duplicados, con mapeo de campos).", capabilities: ["OAuth", "Contactos", "Backfill", "Sin duplicados"] },
    ...
    const delivery = await this.prisma.withTenant(ctx.organizationId, async (tx) => {
      ...
      return tx.webhookDelivery.create({
        data: {
          ...
          endpointId: id,
          event: "test.ping",
          payload: { test: true, message: "Entrega de prueba desde TuBot", occurredAt: new Date().toISOString() },
          status: "PENDING",
        },
      });
    });
